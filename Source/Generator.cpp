...
namespace
{
	...
	int harmonic_smoothing{25}; // Frequencies.
	int temporal_smoothing{2}; // DFTs.
		...
		std::vector<std::vector<float>>* input, int samples, bool harmonic)
	{
		...
		for(int dft_index{}; dft_index < input->size(); ++dft_index)
		{
			...
			{
				// Average the surrounding frequencies.
				int divisor{1};
				float decibels{};
	...
	void generate_dft_data()
	{
		...
		// Validate.
		if(dft_window_size > audio_data.size()) throw std::runtime_error{"The DFT window duration "
			"is greater than the duration of the loaded audio file. Decrease the DFT window duration "
			"or load a longer audio file."};

		if(dft_sample_interval_size+dft_window_size > audio_data.size())
			throw std::runtime_error{"The DFT sample interval will result in less than 2 generated "
			"DFTs. Decrease the DFT sample interval or load a longer audio file."};

		if(harmonic_smoothing > maximum_frequency) throw std::runtime_error{"The harmonic smoothing "
			"value will be greater than the number of frequencies generated by the set DFT window "
			"duration. Decrease the harmonic smoothing value or increase the DFT window duration."};

		if(temporal_smoothing > generated_dft_count) throw std::runtime_error{"The temporal smoothing "
			"value will be greater than the number of generated DFTs. Decrease the temporal smoothing "
			"value or the DFT sample interval, or load a longer audio file."};

		// Warnings.
		const size_t generated_point_count{generated_dft_count*maximum_frequency};

		if(generated_dft_count*maximum_frequency > 10000000) std::cout<<"WARNING: "+std::to_string(
			generated_point_count)+" data points will be generated with this configuration. "
			"This may cause performance issues. You may want to increase the DFT sample interval, "
			"decrease the DFT window size, or load a shorter audio file.\n";

		const size_t sampled_point_count{generated_point_count*
			(harmonic_smoothing+temporal_smoothing)*2};

		if(sampled_point_count > 1000000000) std::cout<<"WARNING: "+std::to_string(sampled_point_count)+
			" data points will be sampled with this configuration. This may cause performance issues. "
			"You may want to decrease the harmonic or temporal smoothing, increase the DFT sample "
			"interval, decrease the DFT window size, or load a shorter audio file.\n";
		...
		while(offset+dft_window_size < audio_data.size())
		{
			...
			{
				const float multiplier{.5f*(1-std::cos(
					2.f*3.1415926f*index/(dft_window_size)))};

				input[index] = multiplier*audio_data[offset+index];
			}
			...
			{
				// Convert the complex frequency data to decibels.
